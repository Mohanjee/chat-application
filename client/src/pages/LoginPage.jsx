    ...
    justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} action="" className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        <h2 className='font-medium text-2xl flex justify-between items-center '>
          ...
          <input onChange={(e) => setFullName(e.target.value)} value = {fullName} autoComplete='name'
          type="text" className='p-2 border border-gray-500 rounded-md focus:outline-none' placeholder='Full Name' required/>
          )}

          {!isDataSubmitted && (
            <>
              <input onChange={(e) => setEmail(e.target.value)} value = {email} autoComplete='email'
               type="email" placeholder='Email Address' required className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-3 focus:ring-indigo-500'/>

              <input onChange={(e) => setPassword(e.target.value)} value = {password} autoComplete={currState === "Sign Up" ? "new-password" : "current-password"}
               type="password" placeholder='Password' required className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-3 focus:ring-indigo-500'/>
          ...
          )}
          {
            currState === "Sign Up" && isDataSubmitted && (
              ...
              className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-3 focus:ring-indigo-500' placeholder='Provide a Short Bio...EX:Using QuickChat' required></textarea>
            )
          }  
