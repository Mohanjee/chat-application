    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      ...
      justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          className={`w-12 h-12 ${selectedImg && 'rounded-full'}`}/> upload profile photo
          </label>
          <input onChange={(e) => setName(e.target.value) } value={name}
           ...
           <textarea name="" id=""  onChange={(e) => setBio(e.target.value)} value={bio}
           placeholder='Write Profile Bio' 
           required  className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500' rows={4}></textarea>
